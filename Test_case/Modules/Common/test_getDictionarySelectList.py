...
class Test_getnotice:
    ...
    @classmethod
    def setup_class(cls):
        path ='/api/carrier/common/getDictionarySelectList'
    ...
    def test_001_getDictionarySelectList(self):
        #入参
        logger = logging.getLogger('test_001_getDictionarySelectList')
        playroad = {"param":{"dictionaryType":"UserStatus"}}
    ...
    def test_002_getDictionarySelectList(self):
        #入参
        logger = logging.getLogger('test_001_getDictionarySelectList')
        playroad = {"param":{"dictionaryType":"DefaultRole"}}
        reponse = Test_getnotice.http.post(self.url,data = json.dumps(playroad),headers = self.headers)
        logger.info(self.url)
        logger.info(self.headers)
        logger.info(json.dumps(playroad))
        ...
        assert resultNote == '0'
        assert resultmessage == '请求成功'
    def test_003_getDictionarySelectList(self):
        #入参
        logger = logging.getLogger('test_001_getDictionarySelectList')
        playroad = {"param":{"dictionaryType":"SettleStatus"}}
        reponse = Test_getnotice.http.post(self.url,data = json.dumps(playroad),headers = self.headers)
        logger.info(self.url)
        logger.info(self.headers)
        logger.info(json.dumps(playroad))
