...
class Test_getnotice:
    ...
    @classmethod
    def setup_class(cls):
        path ='api/login/toSignout'
        cls.url = URLConf.TEST_URL.value + path
        cls.http = Httprequests(cls.url)

    def setup(self):
        Authorizations = read(r'D:\NeustaAutotest\Test_data')
        self.headers = {'sign':'NUFoNQvVLy1l4FzFfsCDydEMkdujR8Dkiqnp99999999','Content-Type':'application/json','Authorization':Authorizations}
        self.http = Httprequests(self.url)

    def teardown(self):
        pass

    def test_001_toSignout(self):
        #入参
        logger = logging.getLogger('test_001_toSignout')
        playroad = {"param":{"loginOutAccessToken":"aa4167ac-1046-497e-825d-f7e1c04aded2"}}
